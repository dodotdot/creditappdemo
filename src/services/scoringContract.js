...
const DEFAULT_TENURE = Math.max(
  1,
  Number(import.meta.env.VITE_SCORING_DEFAULT_TENURE) || 12,
);
...
export function normalizeScoringResponse(raw) {
  ...
  const aegira =
    ...
    Number.isFinite(monthlyInstallment)
      ? {
          currentDsr: Number.isFinite(currentDsr)
            ? Math.round(currentDsr * 100) / 100
            : null,
          projectedDsr: Number.isFinite(projectedDsr)
            ? Math.round(projectedDsr * 100) / 100
            : null,
          riskLevel: riskLevel != null ? String(riskLevel) : null,
          eligible,
          monthlyInstallment: Number.isFinite(monthlyInstallment)
            ? monthlyInstallment
            : null,
        }
      : null;
  ...
  return {
    ...
    internalScore: Math.round(
      readNum(result, "internal_score", "internalScore", 0),
    ),
    breakdown: {
      paymentHistory: Math.round(
        readNum(breakdown, "payment_history", "paymentHistory", 0),
      ),
      income: Math.round(readNum(breakdown, "income", "income", 0)),
      debtToIncome: Math.round(
        readNum(breakdown, "debt_to_income", "debtToIncome", 0),
      ),
      dtiRatio:
    ...
    recommendation: String(
      read(result, "recommendation", "recommendation", ""),
    ),
    traceId: String(read(root, "trace_id", "traceId", "")),
    evaluatedAt: String(
      read(root, "evaluated_at", "evaluatedAt", new Date().toISOString()),
    ),
    aegira,
  };
}
