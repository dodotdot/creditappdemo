# Workshop Workbook — DAVIN BENNETT P N
...
**Jawaban: Tidak ada karena beluma ada proses hit API aslinya**
...
**Jawaban: Di baris 158 -> export async function scoreCreditApplication(input) if (SCORING_MODE === 'uat') return callUatBackend(input); return scoreWithMock(input);**
...
2. Karena unit test biasanya menggunakan Mock API yang mengembalikan respons buatan, sehingga communication antara frontend dengan backend tidak benar-benar terjadi.
3. scoreWithMock() -> melakukan simulasi scoring menggunakan data mock di frontend tanpa memanggil API backend; dan;
callUatBackend() -> mengirim request HTTP ke backend untuk menggunakan respons dari backend.
