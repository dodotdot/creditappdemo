# Workshop Workbook — Muh. Adnan Putra Amiruddin
...
**Jawaban:**
Tidak ada. Karena mock masih di-hit sebagai dummy saja. Selain itu, dicek inspect network web juga belum ada record hit ke link API

**1.2.B** — Field apa di log `[Credit Scoring — API Dummy]`? URL?

**Jawaban:**
![Jawaban 1.2.B](./Jawaban-1.2.B.png)

**1.2.C** — Baris keputusan `mock` vs `uat` di `creditScoring.js`?

**Jawaban:**
Line 158, tepatnya pada function `scoreCreditApplication`
...
3. Kalau `scoreWithMock()` menggunakan data mock/data dummy, sedangkan `callUatBackend()` melakukan call ke API Backend menggunakan method `fetch` dari JavaScript dan datanya bersifat real
